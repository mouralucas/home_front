...
const App = (props: any): React.ReactElement => {
    ...
    * */
    ...
    useEffect(() => {
        if (props.investment) {
            setSelectedParent(parent?.filter((i: any) => i.investmentId === props.investment?.investmentId)[0]);
        }
    ...
    const setParentInvestment = (e: any, name: any, setFunction: any) => {
        ...
        setInvestment((oldValues: any) => ({...oldValues, 'interestRate': parentInvestment?.interestRate}))
        setSelectedInvestmentType(investmentType.filter((i: { value: any; }) => i.value === parentInvestment?.investmentTypeId)[0]);
        setInvestment((oldValues: any) => ({...oldValues, 'investmentTypeId': parentInvestment?.investmentTypeId}))
        setSelectedCustodian(custodian.filter((i: { value: any; }) => i.value === parentInvestment?.custodianId)[0]);
        setInvestment((oldValues: any) => ({...oldValues, 'custodianId': parentInvestment?.custodianId}))
        setInvestment((oldValues: any) => ({...oldValues, 'name': e !== null ? parentInvestment?.name : ''}))
        setInvestment((oldValues: any) => ({...oldValues, 'maturityAt': parentInvestment?.maturityDate}))
        setInvestment((oldValues: any) => ({...oldValues, 'interestIndex': parentInvestment?.interestIndex}))
    }
    ...
    const set = (name: string) => {
        return ({target: {value}}: any) => {
            setInvestment((oldValues: any) => ({...oldValues, [name]: value}));
        }
    }
    ...
    const setCombo = (e: any, name: any, setFunction: any) => {
        if (e !== null) {
            setFunction(e);
            return setInvestment((oldValues: any) => ({...oldValues, [name]: e.value}));

        }
        setFunction(null);
        return setInvestment((oldValues: any) => ({...oldValues, [name]: null}));
    }

    const setDate = (e: any, name: any) => {
        if (e.value !== null) {
            return setInvestment((oldValues: any) => ({...oldValues, [name]: Moment(e.value).format('YYYY-MM-DD')}))
        } else {
            return setInvestment((oldValues: any) => ({...oldValues, [name]: e.value}))
        }
    }
    ...
    const setCurrency = (values: any, name: any) => {
        return setInvestment((oldValues: any) => ({...oldValues, [name]: values.value / 100}));
    }
